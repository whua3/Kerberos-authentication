...
public class Message {
	...
	public static byte[] getContent(byte[] message){
		...
		int i;
		for(i=0; i < length && i+24 < MAX_SIZE; i++){
	 ...
	 * @return
	 */
	public static byte[] getRespondMessage(long tar, long src, byte type, byte respond, byte[] content){
		byte[] message = null;
		if(content != null)
			message= new byte[content.length+24];
		else
			message= new byte[24];
		Message.setTargetID(message, tar);
		Message.setSourceID(message, src);
		Message.setType(message, type);
		Message.setRespond(message, respond);
		if(content != null)
